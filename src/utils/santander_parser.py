...
def processar_extrato_santander_bytes(conteudo_bytes: bytes, nome_arquivo: str = "extrato.xlsx") -> tuple:
    """
    ...
    for col in df.columns:
        ...
        if 'DATA' in col_upper: col_map[col] = 'Data'
        elif any(x in col_upper for x in ['HIST', 'LAN', 'DESC']): col_map[col] = 'Lançamento'
        elif 'CR' in col_upper: col_map[col] = 'Crédito (R$)'
        elif 'DÉB' in col_upper or 'DEB' in col_upper: col_map[col] = 'Débito (R$)'
        elif 'VALOR' in col_upper: col_map[col] = 'Valor'
